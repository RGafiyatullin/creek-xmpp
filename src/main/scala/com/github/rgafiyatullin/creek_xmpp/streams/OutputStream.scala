...
import com.github.rgafiyatullin.creek_xml.common.{HighLevelEvent, Position}
    ...
    streamEvent match {
      ...
      case StreamEvent.StreamOpen(attributes) =>
        val hleStreamOpen = HighLevelEvent.ElementOpen(
          ...
          XmppConstants.names.streams.ns, attributes)

        val nextWriter = HighLevelWriter.empty.in(hleStreamOpen)
        val nextOutputStash = writer.out._1.foldLeft(outputStash)(_.enqueue(_))
